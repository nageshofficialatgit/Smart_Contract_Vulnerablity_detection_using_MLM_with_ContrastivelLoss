...
class ClipmlmTrainer():
    ...
    def train(self, all_dataset, p_dataset):
        ...
        all_dataloader = DataLoader(all_dataset, batch_size=self.args.batch_size_clip, shuffle=True, drop_last=True)
        p_dataloader = DataLoader(p_dataset, batch_size=self.args.batch_size_clip, shuffle=True, drop_last=True)
    ...
    def train_epoch(self, epoch, all_dataloader, p_dataloader):
        ...
        torch.save(torch.cat(plot_data, dim=0), f'./Results/mlm/{self.args.dataset}_{epoch}_data.pt')
        torch.save(torch.cat(plot_label), f'./Results/mlm/{self.args.dataset}_{epoch}_label.pt')
...
class ClipmlmClassifierTrainer():
    def __init__(self, args):
        ...
        if args.resume_file:
            assert os.path.exists(args.resume_file), 'checkpoint not found!'
            logger.info('loading model checkpoint from %s..' % args.resume_file)
            checkpoint = torch.load(args.resume_file, weights_only=False)
            clip.load_state_dict(checkpoint['state_dict'], strict=False)
